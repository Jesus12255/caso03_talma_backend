...
async def saveOrUpdate( request: UsuarioRequest, usuario_facade: UsuarioFacade = Depends(get_usuario_facade), current_user: str = Depends(get_current_user)) -> BaseOperacionResponse:
    return await usuario_facade.saveOrUpdate(request)


@router.get("/init", response_model=UsuarioComboResponse)
async def init( usuario_facade: UsuarioFacade = Depends(get_usuario_facade), current_user: str = Depends(get_current_user)) -> UsuarioComboResponse:
    return await usuario_facade.init()

@router.get("/initForm", response_model=UsuarioComboResponse)
async def initForm( usuario_facade: UsuarioFacade = Depends(get_usuario_facade), current_user: str = Depends(get_current_user)) -> UsuarioComboResponse:
    return await usuario_facade.initForm()


@router.get("/{usuarioId}", response_model=UsuarioResponse)
async def get_usuario(usuarioId: str, usuario_facade: UsuarioFacade = Depends(get_usuario_facade), current_user: str = Depends(get_current_user)) -> UsuarioResponse:
